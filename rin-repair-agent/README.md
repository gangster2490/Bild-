...
В приложении есть встроенная база знаний по ремонту **колясок** и **детских автокресел**
(`prompts/knowledge/` → `assets/knowledge/`).

Разделы: безопасность, инструменты, коляски/приводы, автокресла, стиль RIN.
AI подмешивает релевантные разделы в системный prompt при анализе фото.
Просмотр: главный экран → **База знаний**.
